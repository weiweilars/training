...
class AgentConfig:
    ...
    def __init__(self, config_data: Dict[str, Any]):
        ...
        self.model = self.llm.get("model", "gemini-1.5-flash-latest")
    ...
    async def invoke(self, query: str, session_id: str, user_id: str = "agent_user") -> str:
        ...
        try:
            ...
            if events and events[-1].content and events[-1].content.parts:
                ...
                logger.info(f"ADK agent response generated successfully: {response[:100]}...")
                return response
            else:
                logger.warning(f"No response generated by ADK agent. Events: {len(events)}")
                if events:
                    logger.warning(f"Last event: {events[-1]}")
                    if hasattr(events[-1], 'content'):
                        logger.warning(f"Last event content: {events[-1].content}")
