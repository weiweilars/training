...
try:
    ...
    # from semantic_kernel.connectors.mcp import MCPSsePlugin  # Replaced with custom MCP client
...
# Custom MCP Client (bypassing problematic MCPSsePlugin)
from custom_mcp_client import CustomMCPClient, CustomMCPToolset, create_custom_tool_function
...
class SKAgent:
    ...
    async def create(self, mcp_urls: list = None, preserve_sessions: bool = False):
        ...
        try:
            ...
            # Get all plugins from kernel to pass to agent
            ...
            self._agent = ChatCompletionAgent(
                ...
                plugins=kernel_plugins  # Pass the kernel plugins to the agent
            )
    ...
    def _get_parameter_dicts_from_mcp_tool(self, tool_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Creates parameter dictionaries from MCP tool data (similar to Semantic Kernel's native implementation)"""
    ...
    async def _add_mcp_plugin_to_kernel(self, url: str):
        """Add MCP tools to kernel using custom MCP client with proper parameter registration"""
        try:
            # Import plugin type if available
            from semantic_kernel.functions import KernelPlugin
            
            # Create custom MCP toolset
            custom_toolset = CustomMCPToolset(url)
            
            # Get the full tool metadata (not just names) - this is the key fix!
            tool_metadata_list = await custom_toolset.client.list_tools()
            ...
            for tool_data in tool_metadata_list:
                ...
                def create_kernel_func(name, desc, client, schema_data):
                        ...
                        try:
                            ...
                            if isinstance(result, dict) and "content" in result and len(result["content"]) > 0:
                                ...
                                if content.get("type") == "text":
                                    return content.get("text", json.dumps(result, indent=2))
                    ...
                    # CRITICAL: Add parameter metadata to enable function calling
                    kernel_func.__kernel_function_parameters__ = self._get_parameter_dicts_from_mcp_tool(schema_data)
                    return kernel_func
                
                kernel_func = create_kernel_func(tool_name, tool_description, custom_toolset.client, tool_data)
                plugin_functions[tool_name] = kernel_func
                
                logger.info(f"Registered {tool_name} with {len(kernel_func.__kernel_function_parameters__)} parameters")
            ...
            # Store toolset for reference
            self._tool_plugins[url] = custom_toolset
            
            logger.info(f"Successfully added plugin '{plugin_name}' with {len(tool_metadata_list)} MCP tools from {url} to kernel")
                
        except Exception as e:
            logger.error(f"Failed to add MCP tools from {url}: {e}")
...
# Create FastAPI app
app = FastAPI(title="SK A2A Agent with Custom MCP Client")
