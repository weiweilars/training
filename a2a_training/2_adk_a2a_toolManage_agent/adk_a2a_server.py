...
async def get_agent_card(request):
    ...
    # Build skills dynamically from ADK agent's current tools
    skills = []
    if agent.adk_agent and hasattr(agent.adk_agent, '_tools'):
        for tool_url in agent.adk_agent._tools:
            # Get tool descriptions from history if available
            tool_descriptions = []
            if hasattr(agent.adk_agent, '_tool_history'):
                # Find the most recent addition of this tool for descriptions
                for entry in reversed(agent.adk_agent._tool_history):
                    if entry.get('url') == tool_url and entry.get('action') == 'add':
                        tool_descriptions = entry.get('tool_descriptions', [])
                        break
            
            # Add skills for each tool from this URL
            if tool_descriptions:
                for tool_desc in tool_descriptions:
                    tool_name = tool_desc.get('name', 'unknown_tool')
                    tool_description = tool_desc.get('description', f"MCP tool: {tool_name}")
                    skills.append({
                        "name": tool_name,
                        "description": tool_description,
                        "parameters": tool_desc.get("inputSchema", {}).get("properties", {}),
                        "source_url": tool_url
                    })
            else:
                # Fallback if no tool descriptions available
                skills.append({
                    "name": f"mcp_tool_{len(skills)+1}",
                    "description": f"MCP tool from {tool_url}",
                    "source_url": tool_url
                })
    ...
    agent_card = {
        ...
        "skills": skills,
        "metadata": {
            "mcp_tool_urls": agent.adk_agent._tools if agent.adk_agent and hasattr(agent.adk_agent, '_tools') else [],
            "agent_id": agent.agent_id,
            "status": agent.status,
            "llm_model": "gemini-1.5-flash-latest",
            "adk_powered": True,
            "active_tools_count": len(agent.adk_agent._tools) if agent.adk_agent and hasattr(agent.adk_agent, '_tools') else 0
        }
    }
